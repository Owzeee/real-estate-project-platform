...
import { getInquiriesForDeveloper } from "@/features/inquiries/queries";
import { requireDeveloper } from "@/lib/auth";

export default async function DeveloperInquiriesPage() {
  const auth = await requireDeveloper();
  const inquiries = await getInquiriesForDeveloper(auth.developerProfile.id);
        ...
        <div className="mt-10 grid gap-6">
          {inquiries.length === 0 ? (
            <article className="rounded-[2rem] border border-stone-900/10 bg-white p-8 text-sm text-stone-600 shadow-[0_20px_60px_rgba(41,37,36,0.08)]">
              No inquiries have been submitted for your projects yet.
            </article>
          ) : null}
